...
static Token *parse_typedef(Token *tok, Type *base_type);
...
// example. `int long static` menus the same as `static long int`.
...
static Node *declaration(Token **rest, Token *tok, Type *base_type, VarAttr *attr)
{
  ...
  while (!equal(tok, ";"))
  {
    ...
    if (ty->kind == TY_VLA)
    {
      ...
      // Variable length array (VLAs) are translated to alloca() calls.
      // Form example, `int x[n+2]` is translated to `tmp = n + 2,
      // x = alloca(tmp)`.
...
static void array_designator(Token **rest, Token *tok, Type *ty, int *begin, int *end)
{
  *begin = const_expr(&tok, tok->next);
  if (*begin >= ty->array_len)
    error_tok(tok, "array designator index exceeds arrays bounds");
...
static void initializer2(Token **rest, Token *tok, Initializer *init)
{
  ...
  if (init->ty->kind == TY_STRUCT)
  {
    ...
    // A struct can be initialized with struct. E.g.
...
static Node *stmt(Token **rest, Token *tok)
{
  ...
  if (equal(tok, "for"))
  {
    ...
    if (is_typename(tok))
    {
      ...
      node->init = declaration(&tok, tok, base_type, NULL);
    }
    else
    {
...
static Node *compound_stmt(Token **rest, Token *tok)
{
  ...
  while (!equal(tok, "}"))
  {
    if (is_typename(tok) && !equal(tok->next, ":"))
    {
      ...
      if (attr.is_extern)
      {
        tok = global_variable(tok, base_type, &attr);
        continue;
      }

      cur = cur->next = declaration(&tok, tok, base_type, &attr);
    }
    else
...
//
// In general, `A op= C` is convert to `tmp = &A, *tmp = *tmp op B`.
// However, if a given expression is of form `A.x op=C`, the input is
// convert to `tmp = &A, (*tmp).x = (*tmp).x op C` to handle assignments
// to bitfields.
static Node *to_assign(Node *binary)
{
  ...
  if (binary->lhs->kind == ND_MEMBER)
  {
    ...
    Node *expr2 = new_unary(ND_MEMBER,
                            new_unary(ND_DEREF, new_var_node(var, tok), tok), tok);
...
static Node *conditional(Token **rest, Token *tok)
{
  ...
  if (equal(tok->next, ":"))
  {
    // [GUN] Compile `a ?: b` as `tmp = a, tmp ? tmp : b`.
    add_type(cond);
...
static void struct_members(Token **rest, Token *tok, Type *ty)
{
  ...
  while (!equal(tok, "}"))
  {
    ...
    // Anonymous struct member
    if ((base_type->kind == TY_STRUCT || base_type->kind == TY_UNION) &&
        consume(&tok, tok, ";"))
    {
    ...
    while (!consume(&tok, tok, ";"))
    {
      ...
      Member *mem = calloc(1, sizeof(Member));
      mem->ty = declarator(&tok, tok, base_type);
...
// have another unnamed struct as a member as a member like this:
//
...
static void resolve_goto_labels(void)
{
  for (Node *x = gotos; x; x = x->goto_next)
  {
    ...
    if (x->unique_label == NULL)
      error_tok(x->tok->next, "use of declared label");
  }
...
static Token *global_variable(Token *tok, Type *base_type, VarAttr *attr)
{
  bool first = true;
...
static void scan_globals(void)
{
  ...
  for (Obj *var = globals; var; var = var->next)
  {
    ...
    // If there's another definiton, the tentative definition
    // is redundant
    if (!var2)
      cur = cur->next = var;
  }
