...
static Token *read_const_expr(Token **rest, Token *tok)
{
  ...
  while (tok->kind != TK_EOF)
  {
    // "defined(foo)" or "define foo" becomes "1" if macro "foo"
...
static long eval_const_expr(Token **rest, Token *tok)
{
  ...
  // Convert pp-number to regular numbers
...
// This function is used for the stringzing operator (#).
...
static Token *subst(Token *tok, MacroArg *args)
{
  ...
  while (tok->kind != TK_EOF)
  {
    ...
    // before the are substituted into a macro body.
    if (arg)
    {
...
// Otherwise, do noting and return false.
...
static Token *preprocess2(Token *tok)
{
  ...
  while (tok->kind != TK_EOF)
  {
    ...
    error_tok(tok, "invalid preprocess directive");
  }
...
static void join_adjacent_string_literals(Token *tok)
{
  ...
  // type before concatenation. In this case, we do the conversion.
  for (Token *tok1 = tok; tok1->kind != TK_EOF;)
  {
    ...
    if (base_type->size > 1)
      for (Token *t = tok1; t->kind == TK_STR; t = t->next)
        if (t->ty->base->size == 1)
          *t = *tokenize_string_literal(t, base_type);
