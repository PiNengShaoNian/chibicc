  ...
  case ND_VAR:
    ...
    if (node->ty->kind == TY_FUNC)
    {
      if (node->var->is_definition)
        println("  lea %s(%%rip), %%rax  # (function)", node->var->name);
      else
        println("  mov %s@GOTPCREL(%%rip), %%rax  # (function)", node->var->name);
      return;
    }
...
static void load(Type *ty)
{
  ...
  case TY_VLA:
    ...
    // This is where "array is automatically concerted to a pointer to
...
// they are passed in a XMM register. Otherwise, they are passed
// in a general-purpose register.
//
// If a struct/union is larger than 8 bytes, the same rule is
// applied to the next 8 byte chunk.
//
...
static int push_args(Node *node)
{
  ...
  // Load as many arguments to the register as possible.
...
static void gen_expr(Node *node)
{
  ...
  case ND_FUNCALL:
  {
    ...
    // If the return type is a large struct/union, the caller passes
    // a pointer the a buffer as if it were the first argument.
    if (node->ret_buffer && node->ty->size > 16)
      pop(argreg64[gp++]);
    ...
    // If the return tye is a small struct, a value is returned
    // using up to two registers.
    if (node->ret_buffer && node->ty->size <= 16)
    {
...
static void gen_stmt(Node *node)
{
  ...
  case ND_SWITCH:
    ...
    for (Node *n = node->case_next; n; n = n->case_next)
    {
      ...
      println("  sub $%ld, %s", n->begin, di); // di = expr - begin
...
static void assign_lvar_offsets(Obj *prog)
{
  for (Obj *fn = prog; fn; fn = fn->next)
  {
    ...
    for (Obj *var = fn->locals; var; var = var->next)
    {
      ...
      // AMD64 System V ABI has a special alignment rule for any array of
      // length at least 16 bytes. We need to align such array to at least
      // by-byte boundaries. See p.14 of
